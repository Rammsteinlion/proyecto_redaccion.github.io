document.addEventListener("DOMContentLoaded", () => {
  ...
  fileInput.addEventListener("change", async (e) => {
    ...
    reader.onload = (event) => {
      ...
      workbook.SheetNames.forEach((n) => {
        res[n.toUpperCase()] = XLSX.utils.sheet_to_json(workbook.Sheets[n], {
          header: 1,
        });
      });
  ...
  function obtenerOrientacionDominante(b) {
    ...
    const centroLinderoN =
      puntosLindero.reduce((a, p) => a + p.norte, 0) / puntosLindero.length;
    const centroLinderoE =
      puntosLindero.reduce((a, p) => a + p.este, 0) / puntosLindero.length;
  ...
  function generarRedaccion() {
    ...
    const todosLosIds = Object.keys(pointsDataMap)
      .map(Number)
      .sort((a, b) => a - b);
    ...
    boundariesData.forEach((b, idx) => {
      ...
      const esLinderoCierre =
        b.pInicio === puntoMaximo && b.pFin === puntoMinimo;
      ...
      if (esLinderoCierre) {
        ...
        for (let i = 0; i < recorrido.length - 1; i++) {
          const actualId = recorrido[i];
          const siguienteId = recorrido[i + 1];

          const p1 = pointsDataMap[actualId];
          const p2 = pointsDataMap[siguienteId];
          if (!p1 || !p2) continue;

          const sentido = obtenerSentidoCartesiano(p1, p2);

          // Inicializa sentido
          if (tramoActual.sentido === "") {
            tramoActual.sentido = sentido;
          }

          // 🔥 CAMBIO DE SENTIDO = NUEVO TRAMO
          if (tramoActual.sentido !== "" && sentido !== tramoActual.sentido) {
            tramoActual.fin = actualId;
          ...
          // 🔥 evitar duplicados
          if (siguienteId !== tramoActual.fin) {
            tramoActual.puntos.push(siguienteId);
          }
        }
      ...
      tramos.forEach((tr, index) => {
        ...
        const intermedios = tr.puntos.filter((p) => p !== tr.fin);
        const tieneIntermedios = intermedios.length > 0;

        // 🔥 siempre quebrada (como tu formato real)
        parrafo += `en línea quebrada en sentido ${tr.sentido}, `;

        // 🔥 SOLO si hay puntos → se agrega "pasando por"
        if (tieneIntermedios) {
          const lista = intermedios.map(
            (p) =>
              `el punto número ${p} de coordenadas planas N= ${pointsDataMap[p].norte.toFixed(2)}m, E= ${pointsDataMap[p].este.toFixed(2)}m`,
          );

          parrafo += `pasando por ${lista.join(", ")}, `;
        }
  ...
  function mostrarBotonGenerar() {
    ...
    btn.className =
      "w-full py-3 bg-blue-600 text-white rounded-xl font-bold mb-4";
  ...
  function dibujarPlano() {
    ...
    const puntosOrdenados = Object.entries(pointsDataMap).sort(
  ...
  function mostrarBotonDescarga(texto) {
    ...
    btn.id = "btnWord";
    btn.className =
      "w-full py-3 bg-green-600 text-white rounded-xl font-bold shadow-lg hover:bg-green-700";
